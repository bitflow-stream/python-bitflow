...
class TestFileIO(unittest.TestCase):
    ...
    def test_csv_file_in_no_out(self):
        pipeline = bitflow_pipeline.Pipeline()
        pipeline.start()
    ...
    def test_bin_file_in_no_out(self):
        pipeline = bitflow_pipeline.Pipeline()
        pipeline.start()
        file_source = source.FileSource(filename=TESTING_IN_FILE_BIN,
                                        pipeline=pipeline)
        file_source.start()
        time.sleep(self.DEFAULT_SLEEPING_DURATION)
        pipeline.stop()
    ...
    def test_csv_file_in__bin_file_out(self):
        ...
        pipeline.add_processing_step(sinksteps.FileSink(filename=TESTING_OUT_FILE_BIN,
                                                        data_format=script_parser.BINARY_DATA_FORMAT_IDENTIFIER))
        pipeline.start()
        file_source = source.FileSource(filename=TESTING_IN_FILE_CSV,
                                        pipeline=pipeline)
        file_source.start()
    ...
    def test_csv_file_in__multiple_csv_files_out(self):
        ...
        pipeline.add_processing_step(sinksteps.FileSink(  filename=TESTING_OUT_FILE_CSV_2,
                                                data_format=script_parser.CSV_DATA_FORMAT_IDENTIFIER))
        pipeline.start()
        file_source = source.FileSource(filename=TESTING_IN_FILE_CSV,
                                        pipeline=pipeline)
...
class TestTcpIO(unittest.TestCase):
    ...
    def test_csv_listen_in__csv_send_out(self):
        ...
        a_pipeline = bitflow_pipeline.Pipeline()
        a_pipeline.add_processing_step(sinksteps.FileSink(filename=TESTING_OUT_FILE_CSV,
                                                data_format=script_parser.CSV_DATA_FORMAT_IDENTIFIER))
        a_listen_source = source.ListenSource(  pipeline=a_pipeline,
                                                port=port)
        a_pipeline.start()
        a_listen_source.start()

        # BUILD FILE TO SEND
        b_pipeline = bitflow_pipeline.Pipeline()
        b_pipeline.add_processing_step(sinksteps.TCPSink(
                            host=host,
                            port=port))
        b_file_source = source.FileSource( filename=TESTING_IN_FILE_CSV,
                                            pipeline=b_pipeline)
        b_pipeline.start()
        b_file_source.start()
    ...
    def test_bin_listen_in__csv_send_out(self):
        ...
        b_pipeline.add_processing_step(sinksteps.TCPSink(
                            host=host,
                            port=port))
        b_file_source = source.FileSource(  filename=TESTING_IN_FILE_BIN,
                                            pipeline=b_pipeline)
        b_pipeline.start()
        b_file_source.start()
    ...
    def test_csv_listen_in__bin_send_out(self):
        ...
        # BUILD LISTEN TO FILE
        a_pipeline = bitflow_pipeline.Pipeline()
        a_pipeline.add_processing_step(sinksteps.FileSink(  filename=TESTING_OUT_FILE_BIN,
                                                            data_format=script_parser.BINARY_DATA_FORMAT_IDENTIFIER))
        a_listen_source = source.ListenSource( pipeline=a_pipeline,
                                        port=port)
        a_pipeline.start()
    ...
    def test_csv_download_in__csv_listen_out(self):
        ...
        a_pipeline = bitflow_pipeline.Pipeline()
        a_file_source = source.FileSource(  filename=TESTING_IN_FILE_CSV,
                                            pipeline=a_pipeline)
        a_pipeline.add_processing_step(sinksteps.ListenSink( max_receivers=5,
                                                            host=host,
                                                            port=port))
        a_file_source.start()
        a_pipeline.start()
        b_pipeline = bitflow_pipeline.Pipeline()
        b_download_source = source.DownloadSource( host=host,
                                            port=port,
                                            pipeline=b_pipeline)
        b_pipeline.add_processing_step(sinksteps.FileSink(  filename=TESTING_OUT_FILE_CSV,
                                                            data_format=script_parser.CSV_DATA_FORMAT_IDENTIFIER))
        b_pipeline.start()
    ...
    def test_bin_download_in__csv_listen_out(self):
        ...
        b_download_source = source.DownloadSource( host=host,
                                            port=port,
                                            pipeline=b_pipeline)
        b_pipeline.add_processing_step(sinksteps.FileSink(  filename=TESTING_OUT_FILE_BIN,
                                                            data_format=script_parser.BINARY_DATA_FORMAT_IDENTIFIER))
        b_pipeline.start()
        b_download_source.start()
        time.sleep(self.DEFAULT_SLEEPING_DURATION)
    ...
    def test_csv_download_in__bin_listen_out(self):
        ...
        a_pipeline = bitflow_pipeline.Pipeline()
        a_file_source = source.FileSource(  filename=TESTING_IN_FILE_BIN,
                                            pipeline=a_pipeline)
        a_pipeline.add_processing_step(sinksteps.ListenSink(max_receivers=5,
                                                            host=host,
                                                            port=port))
