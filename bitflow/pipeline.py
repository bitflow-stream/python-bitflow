...
class Pipeline_Termination(ProcessingStep):
...
class Pipeline(Thread):
    ...
    def __init__(self, maxsize=DEFAULT_QUEUE_MAXSIZE, multiprocessing_input=DEFAULT_MULTIPROCESSING_INPUT):
        ...
        self.pipeline_termination = Pipeline_Termination(self.sample_queue_out)
        self.processing_steps = []

    def create_queue(self, multiprocessing_input):
        if multiprocessing_input:
            sample_queue = multiprocessing.Queue(maxsize=self.maxsize)
        else:
...
class BatchPipeline(Pipeline):
    ...
    def execute_first_step(self):
        try:
            ...
            if isinstance(self.sample_queue_in, thread_queue.Queue):
