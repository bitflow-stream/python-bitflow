...
class PlotTagBoxplot(ProcessingStep):
    ...
    __description__ = "Boxplot processing step. Plots given metric and distinguashes by given tag name. " \
                      "One box per unique tag_value. file_formats: png|pdf|svg(default=png)"
    ...
    def __init__(self,
        ...
        if file_format.lower() in self.SUPPORTED_FILE_FORMATS:
            ...
            raise NotSupportedWarning("{}: file format, {}, not supported ...".format(self.__name__, file_format))
    ...
    def execute(self, sample):
        ...
        try:
            index = sample.header.metric_names.index(self.metric_name)  # find index of metric
        except ValueError:
            self.write(sample)
            return

        if self.tag == "":
            if self.metric_name not in self.values:
...
class PlotLinePlot(ProcessingStep):
    ...
    __description__ = "Lineplot processing step. Plots given metrics E.g. one line per given " \
                      "metric over time. file_format: png|pdf|svg(default=png)"
    ...
    def __init__(self,
        ...
        if filename == "":
            self.filename = str(self.__name__) + "-" + str(self.metric_names[0]) + "." + str(file_format)
        else:
            self.filename = filename
        if ylabel == "":
            self.ylabel = None
...
class PlotBarPlot(ProcessingStep):
    ...
    __description__ = "Barplot processing step. Plots given metrics as bar. using aggregation: " \
                      "sum|avg(default=sum), file_format: png|pdf|svg(default=png)"
    ...
    def execute(self, sample):
        ...
        for metric in self.metric_names:
            try:
                index = sample.header.metric_names.index(metric)  # find index of metric
            except ValueError:
                self.write(sample)
                return
